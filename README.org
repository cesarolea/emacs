...
(defun my/font-config ()
  ...
  ;; Font configuration
  (set-face-attribute 'default nil :font "Iosevka Nerd Font Mono" :height (if (equal system-type 'darwin) 190 110) :weight 'regular)

  ;; Set the fixed pitch face
  (set-face-attribute 'fixed-pitch nil :font "Iosevka Nerd Font Mono" :height (if (equal system-type 'darwin) 180 120))

  ;; Set the variable pitch face
  (set-face-attribute 'variable-pitch nil :font "Iosevka Aile" :height (if (equal system-type 'darwin) 180 130) :weight 'regular))
#+end_src
...
(define-key global-map (kbd "<f7>") 'my/ansi-term)
