...
EasyPG comes built into Emacs. It's an interface to GnuPG. The configuration is to fix a compatibility issue with the newer GnuPG 2.4.3.

See: https://dev.gnupg.org/T6481
...
(defun my/font-config ()
  ...
  ;; Font configuration
  (set-face-attribute 'default nil :font "Iosevka NFM" :height (if (equal system-type 'darwin) 190 120) :weight 'regular)

  ;; Set the fixed pitch face
  (set-face-attribute 'fixed-pitch nil :font "Iosevka NFM" :height (if (equal system-type 'darwin) 180 130))
...
Vertico comes with some extensions. 

*** Vertico directory

Provides ido-like directory navigation
...
(use-package company
  ...
  (global-set-key (kbd "C-c '") 'company-abbrev)
  ;(global-company-mode)
...
Display an outline of the current document. Useful when working with a large document and you want to see its outline.

#+begin_src emacs-lisp
(use-package org-sidebar
  :straight (:host github :repo "alphapapa/org-sidebar")
  :bind (:map org-mode-map ("C-c C-x b" . org-sidebar-tree-toggle)))
#+end_src
...
(use-package org-modern
  ...
  (custom-theme-set-faces
   ...
   ;; '(org-verbatim              ((t (:inherit (shadow fixed-pitch)))))
      ...
      '(org-modern-symbol         ((t (:family "Iosevka Aile"))) t)
      '(org-modern-done           ((t (:inherit org-modern-label
                                       :family fixed-pitch
                                       :background "gray20"
                                       :foreground "white"))) t)
...
(use-package org-roam
  ...
  :config
  ;; override default note capture template to add visual line mode
...
(use-package fireplace
  :config
  (setq fireplace-sound-file-path "~/.config/emacs/fireplace.mp3"))
#+end_src
...
The following package takes care of managing the language parsers, that way you don't have to do it manually:
...
(use-package lsp-mode
  ...
  ;;       lsp-elixir-mix-target "/opt/homebrew/bin/mix"
  ;;       lsp-clojure-custom-server-command (if (equal system-type 'darwin)
...
(use-package lsp-ui
  ...
  :bind (:map lsp-ui-mode-map ("C-'" . 'lsp-ui-doc-glance))
  :config
...
(use-package cider
  ...
  (setq nrepl-hide-special-buffers t
        ...
        cider-reuse-dead-repls 'auto                           ; don't prompt to reuse dead buffers, just do it
        cider-merge-sessions 'project                          ; workaround to group repl sessions originating from different config files (e.g. deps and shadow-cljs)
        cider-clojure-cli-global-aliases ":1.11-storm"         ; default alias to load flowstorm when jack-in
        )

  ;; Uncomment the code below to prevent the error buffer from popping up
...
;; commenting out because it pulls multiple-cursors. Let's see if I miss it.
...
~hledger~ is another plain text accounting (PTA) utility, similar to [[*Ledger][Ledger]], but written in Haskell and with additional features

#+begin_src emacs-lisp
(use-package hledger-mode
  :mode ("\\.journal\\'")
  :hook (hledger-mode . abbrev-mode)
  :bind (:map hledger-mode-map ("C-'" . 'hledger-company))
  :init
  (setq hledger-jfile (expand-file-name "~/Dropbox/finance/hledger.journal"))
  :config
...
[[https://www.reverso.net][Reverso]] is an AI-driven translation service. The ~reverso.el~ package integrates that service directly in Emacs.

#+begin_src emacs-lisp
(use-package reverso
  :straight (:host github :repo "SqrtMinusOne/reverso.el")
  :bind ("<f11>" . reverso-translate)
  :config
  (setq reverso-languages '(italian spanish)))
#+end_src
...
(use-package mu4e
  ...
  (setq mu4e-change-filenames-when-moving t
        ...
        ;; mu4e-read-option-use-builtin nil               ;; don't use mu4e completion
        ;; mu4e-completing-read-function 'completing-read ;; use emacs completion
        mail-user-agent 'mu4e-user-agent                  ;; mu4e as default mail client
        read-mail-command 'mu4e
        mu4e-support-modeline nil
        )
        ...
        (list
         (make-mu4e-context
          ...
          :vars '((user-mail-address . "cesar.olea@loanpro.io")
                  ...
                  (message-signature-file "~/.config/mu4e-signature")
                  (message-signature . t)
                  (mu4e-compose-signature . "<#part type=text/html><div dir=\"ltr\"><div style=\"color:rgb(0,0,0);font-family:Arial,&quot;Helvetica Neue&quot;,Helvetica,sans-serif;font-size:medium;float:left;padding:2px 5px 2px 0px\"><img src=\"https://ci3.googleusercontent.com/meips/ADKq_NYDTR9u-a55V33GRiNM8OX4aWRIowtf55ExHlRUmKMiog9dbO2tHuSFlz1AsD3-roJ3iCbKq0A6dWItun_yFg1kmfqXM19yU2m5Rn7vdWGDILC_TUj86VLdkbqaNV7NKym8I_olS370y-tzKIuVh--r8cK-4LceXUVcQbFRrbpvdUAhB5Uv7Ph2fewO6Xq5yhrQslFFrSBm=s0-d-e1-ft#https://static.helpjuice.com/helpjuice_production/uploads/upload/image/12133/4256059/loanpro-favicon-60x60-rounded-edge%2B_1_.png\" style=\"width:60px\" class=\"CToWUd\" data-bit=\"iit\"></div><div style=\"color:rgb(0,0,0);font-family:Arial,&quot;Helvetica Neue&quot;,Helvetica,sans-serif;font-size:medium;float:left;padding-left:10px;margin-top:4px\"><p style=\"margin:0px;padding:0px;font-size:15px;font-weight:bold\">César Olea</p><p style=\"margin:0px;color:rgb(82,51,237);padding:0px;font-size:12px\">Chief Technology Officer</p><div style=\"padding:0px;margin:0px;font-size:12px\"><span><a href=\"https://loanpro.io/\" style=\"color:rgb(0,0,0);padding-right:5px\" target=\"_blank\" data-saferedirecturl=\"https://www.google.com/url?q=https://loanpro.io/&amp;source=gmail&amp;ust=1722102193469000&amp;usg=AOvVaw26JyG1iVLDQYi02TBzSlao\">loanpro.io</a></span></div></div></div><#/part>")
  ...
  (setq mu4e-bookmarks
        '(("maildir:\"/loanpro/Inbox\" AND NOT flag:trashed"                                                     "LoanPro Inbox"           ?p)
          ...
          ("flag:unread AND NOT flag:trashed AND NOTv(maildir:\"/fastmail/Sent\" OR maildir:\"/fastmail/Spam\" OR maildir:\"/loanpro/[Gmail]/Trash\")"    "Unread messages"         ?i)
...
- ~blink-cursor-mode~ Enable cursor blinking, and set it to red.
