...
*** Theme
 ...
 (use-package doom-themes :pin melpa
   ...
   ;; Font configuration
   (set-face-attribute 'default nil :font "IBM Plex Mono" :height (if (equal system-type 'darwin) 150 110) :weight 'regular)
...
#+begin_src emacs-lisp
(defun my/ansi-term ()
  "Start fish shell in a terminal emulator. Like `ansi-term', but respect buffer display actions."
  (interactive)
...
Help buffers should also go to a side window: at the bottom if there's already a side window there or at the right if there's no side window displayed yet. Prevents multiple help buffers from popping up by reusing the same window.
...
(use-package mu4e
  ...
  (setq mu4e-bookmarks
        '(("maildir:\"/loanpro/Inbox\" AND NOT flag:trashed"                                                     "LoanPro Inbox"           ?p)
          ...
          ("flag:unread AND NOT (flag:trashed OR maildir:\"/fastmail/Sent\" OR maildir:\"/fastmail/Spam\")"      "Unread messages"         ?i)
